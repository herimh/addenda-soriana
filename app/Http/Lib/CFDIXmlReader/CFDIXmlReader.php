...
class CFDIXmlReader 
{
    ...
    public static function getArrayFromDir($dir){
        return self::getInstance()->getFromDir($dir);
    }

    public function getFromFile($file){
        if (file_exists($file) && mime_content_type($file) == self::FILE_TYPE_XML){
            $cfdi = new CfdiParser\Parser($file);
            return $cfdi->getCfdiArray();
        }
    ...
    public static function getArrayFromFile($file){
        return self::getInstance()->getFromFile($file);
    }
